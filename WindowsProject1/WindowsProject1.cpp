#include <windows.h>
#include <commctrl.h> 
#include <cstdlib>
#include "resource.h"
#include <iostream>
#include <fstream>
#include <string>
#include <vector>
#include <commdlg.h>
#include <gdiplus.h>
#include <Uxtheme.h>
#include <algorithm>


#pragma comment(lib, "uxtheme.lib")

#pragma comment(lib, "gdiplus.lib")

#pragma comment(lib, "comctl32.lib")

#pragma comment(linker,"\"/manifestdependency:type='win32' \
name='Microsoft.Windows.Common-Controls' version='6.0.0.0' \
processorArchitecture='*' publicKeyToken='6595b64144ccf1df' language='*'\"")

HFONT g_hTitleFont = NULL;

#ifndef VARIABLE_ATTRIBUTE_NON_VOLATILE
#define VARIABLE_ATTRIBUTE_NON_VOLATILE       0x00000001
#endif
#ifndef VARIABLE_ATTRIBUTE_BOOTSERVICE_ACCESS
#define VARIABLE_ATTRIBUTE_BOOTSERVICE_ACCESS 0x00000002
#endif
#ifndef VARIABLE_ATTRIBUTE_RUNTIME_ACCESS
#define VARIABLE_ATTRIBUTE_RUNTIME_ACCESS     0x00000004
#endif

using namespace std;

Gdiplus::Image* LoadPNGFromResource(HMODULE hMod, HRSRC hRes) {
    DWORD size = SizeofResource(hMod, hRes);
    HGLOBAL hMem = LoadResource(hMod, hRes);
    void* pData = LockResource(hMem);
    HGLOBAL hBuffer = GlobalAlloc(GMEM_MOVEABLE, size);
    void* pBuffer = GlobalLock(hBuffer);
    CopyMemory(pBuffer, pData, size);
    GlobalUnlock(hBuffer);
    IStream* pStream = NULL;
    CreateStreamOnHGlobal(hBuffer, TRUE, &pStream);
    return Gdiplus::Image::FromStream(pStream);
}

struct KeyboardInfo {
    wstring name;
    wstring id;
};
vector<KeyboardInfo> g_keyboardList;

RECT g_lastDlgRect = { 0 };
string g_selectedOSInfo = "Unknown";

void CenterWindowOnScreen(HWND hWnd) {
    RECT rcDlg, rcScreen;
    GetWindowRect(hWnd, &rcDlg);

    int dlgWidth = rcDlg.right - rcDlg.left;
    int dlgHeight = rcDlg.bottom - rcDlg.top;

    int screenWidth = GetSystemMetrics(SM_CXSCREEN);
    int screenHeight = GetSystemMetrics(SM_CYSCREEN);

    int x = (screenWidth - dlgWidth) / 2;
    int y = (screenHeight - dlgHeight) / 2;

    SetWindowPos(hWnd, NULL, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
}

INT_PTR CALLBACK AccessDeniedDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_INITDIALOG) {
        CenterWindowOnScreen(hDlg);
        return (INT_PTR)TRUE;
    }
    if (message == WM_COMMAND) {
        int wmId = LOWORD(wParam);
        if (wmId == IDOK || wmId == IDCANCEL) {
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;
        }
    }
    return (INT_PTR)FALSE;
}

void addCornerIcon(HWND hDlg, WORD iconId) {
    HINSTANCE hInstance = (HINSTANCE)GetWindowLongPtr(hDlg, GWLP_HINSTANCE);
    HICON hSmallIcon = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
    HICON hBigIcon = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR);

    if (hSmallIcon) {
        SendMessage(hDlg, WM_SETICON, ICON_SMALL, (LPARAM)hSmallIcon);
    }
    if (hBigIcon) {
        SendMessage(hDlg, WM_SETICON, ICON_BIG, (LPARAM)hBigIcon);
    }
}

INT_PTR CALLBACK LangDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        addCornerIcon(hDlg, IDI_ICON1);

        HWND hLangCombo = GetDlgItem(hDlg, IDC_COMBO1);
        if (hLangCombo) {
            SendMessage(hLangCombo, CB_ADDSTRING, 0, (LPARAM)L"English (United States)");
            SendMessage(hLangCombo, CB_SETCURSEL, 0, 0);
        }

        HWND hCombo = GetDlgItem(hDlg, IDC_COMBO2);
        if (!hCombo) return (INT_PTR)TRUE;

        HKEY hKey;
        if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\Keyboard Layouts", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            WCHAR subKeyName[256];
            DWORD subKeySize = 256;
            DWORD index = 0;

            g_keyboardList.clear();

            while (RegEnumKeyExW(hKey, index, subKeyName, &subKeySize, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
                HKEY hSubKey;
                wstring fullPath = L"SYSTEM\\CurrentControlSet\\Control\\Keyboard Layouts\\" + wstring(subKeyName);

                if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, fullPath.c_str(), 0, KEY_READ, &hSubKey) == ERROR_SUCCESS) {
                    WCHAR layoutText[256];
                    DWORD layoutTextSize = sizeof(layoutText);

                    if (RegQueryValueExW(hSubKey, L"Layout Text", NULL, NULL, (LPBYTE)layoutText, &layoutTextSize) == ERROR_SUCCESS) {
                        KeyboardInfo ki;
                        ki.name = layoutText;
                        ki.id = subKeyName;
                        g_keyboardList.push_back(ki);
                    }
                    RegCloseKey(hSubKey);
                }
                subKeySize = 256;
                index++;
            }
            RegCloseKey(hKey);
        }

        LRESULT usKeyboardIdx = 0;
        for (size_t i = 0; i < g_keyboardList.size(); ++i) {
            wstring displayName = g_keyboardList[i].name;
            LRESULT idx = SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)displayName.c_str());
            SendMessage(hCombo, CB_SETITEMDATA, idx, (LPARAM)i);
        }

        LRESULT totalCount = SendMessage(hCombo, CB_GETCOUNT, 0, 0);

        for (LRESULT idx = 0; idx < totalCount; ++idx) {
            LRESULT dataIdx = SendMessage(hCombo, CB_GETITEMDATA, idx, 0);

            if (dataIdx != CB_ERR && dataIdx >= 0 && dataIdx < (LRESULT)g_keyboardList.size()) {
                if (g_keyboardList[dataIdx].id == L"00000409") {
                    usKeyboardIdx = idx;
                    break;
                }
            }
        }
        SendMessage(hCombo, CB_SETCURSEL, usKeyboardIdx, 0);

        return (INT_PTR)TRUE;
    }

    case WM_COMMAND:
    {
        int wmId = LOWORD(wParam);
        if (wmId == IDOK)
        {
            HWND hCombo = GetDlgItem(hDlg, IDC_COMBO2);
            LRESULT selIdx = SendMessage(hCombo, CB_GETCURSEL, 0, 0);
            if (selIdx != CB_ERR) {

                LRESULT dataIdx = SendMessage(hCombo, CB_GETITEMDATA, selIdx, 0);


                if (dataIdx >= 0 && dataIdx < (LRESULT)g_keyboardList.size()) {
                    wstring targetID = g_keyboardList[dataIdx].id;


                    HKL hkl = LoadKeyboardLayoutW(targetID.c_str(), KLF_ACTIVATE);
                    if (hkl != NULL) {
                        ActivateKeyboardLayout(hkl, 0);

                        PostMessageW(HWND_BROADCAST, WM_INPUTLANGCHANGEREQUEST, 0, (LPARAM)hkl);
                    }
                }
            }
            EndDialog(hDlg, IDOK);
            return (INT_PTR)TRUE;
        }
        else if (wmId == IDCANCEL)
        {
            EndDialog(hDlg, IDCANCEL);
            return (INT_PTR)TRUE;
        }
        break;
    }
    case WM_DESTROY:
    {
        GetWindowRect(hDlg, &g_lastDlgRect);
        break;
    }

    }

    CenterWindowOnScreen(hDlg);
    return (INT_PTR)FALSE;
}

struct OSInfo {
    wstring name;
    wstring driveLetter;
    wstring sizeMB;
    wstring guid;
};

wstring GetPartitionSizeInMB(wchar_t driveLetter) {
    wchar_t rootPath[] = { driveLetter, L':', L'\\', L'\0' };
    ULARGE_INTEGER freeBytesAvailableToCaller, totalNumberOfBytes, totalNumberOfFreeBytes;

    if (GetDiskFreeSpaceExW(rootPath, &freeBytesAvailableToCaller, &totalNumberOfBytes, &totalNumberOfFreeBytes)) {
        ULONGLONG totalMB = totalNumberOfBytes.QuadPart / (1024 * 1024);
        return to_wstring(totalMB) + L" MB";
    }
    return L"Unknown";
}


vector<OSInfo> ScanInstalledOSList() {
    vector<OSInfo> osList;

    wchar_t tempPath[MAX_PATH];
    wchar_t tempFile[MAX_PATH];
    GetTempPathW(MAX_PATH, tempPath);
    swprintf_s(tempFile, L"%sbcdinfo.txt", tempPath);

    wchar_t cmd[MAX_PATH * 2];
    swprintf_s(cmd, L"cmd.exe /c bcdedit /v /enum osloader > \"%s\"", tempFile);

    STARTUPINFOW si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    if (CreateProcessW(NULL, cmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        WaitForSingleObject(pi.hProcess, 2000);
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }

    wifstream file(tempFile);
    if (!file.is_open()) return osList;

    wstring line;
    OSInfo currentOS;
    bool hasOS = false;

    while (getline(file, line)) {
        size_t idPos = line.find(L"identifier");
        if (idPos != wstring::npos) {
            wstring val = line.substr(idPos + 10);
            size_t first = val.find_first_not_of(L" \t\r\n");
            size_t last = val.find_last_not_of(L" \t\r\n");
            if (first != wstring::npos && last != wstring::npos) {
                currentOS.guid = val.substr(first, (last - first + 1));
            }
        }

        size_t partPos = line.find(L"partition=");
        if (partPos != wstring::npos && partPos + 10 < line.length()) {
            wchar_t letter = line[partPos + 10];
            currentOS.driveLetter = L"(" + wstring(1, towupper(letter)) + L":) Local Disk";
            currentOS.sizeMB = GetPartitionSizeInMB(towupper(letter));
        }

        size_t descPos = line.find(L"description");
        if (descPos != wstring::npos) {
            wstring val = line.substr(descPos + 11);
            size_t first = val.find_first_not_of(L" \t\r\n");
            size_t last = val.find_last_not_of(L" \t\r\n");
            if (first != wstring::npos && last != wstring::npos) {
                currentOS.name = val.substr(first, (last - first + 1));
                hasOS = true;
            }
        }

        if (line.empty() && hasOS) {
            osList.push_back(currentOS);
            currentOS = OSInfo();
            hasOS = false;
        }
    }

    if (hasOS) {
        osList.push_back(currentOS);
    }

    file.close();
    DeleteFileW(tempFile);
    return osList;
}

//Avoid cmd to pop up
void RunCommand(const char* cmd) {

    STARTUPINFOA si;
    PROCESS_INFORMATION pi;

    ZeroMemory(&si, sizeof(si));
    si.cb = sizeof(si);
    ZeroMemory(&pi, sizeof(pi));

    char cmdBuffer[MAX_PATH * 2];
    strcpy_s(cmdBuffer, cmd);

    if (CreateProcessA(
        NULL,
        cmdBuffer,
        NULL,
        NULL,
        FALSE,
        CREATE_NO_WINDOW,
        NULL,
        NULL,
        &si,
        &pi
    )) {
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }
}

vector<OSInfo> g_osListForBootDefaults;

void UpdateUIState(HWND hDlg) { //Only used in dialog 2
    BOOL r1 = (IsDlgButtonChecked(hDlg, IDC_RADIO1) == BST_CHECKED);
    BOOL r2 = (IsDlgButtonChecked(hDlg, IDC_RADIO2) == BST_CHECKED);

    HWND hList = GetDlgItem(hDlg, IDC_LIST3);
    HWND hNextBtn = GetDlgItem(hDlg, IDC_BUTTON2);

    EnableWindow(hList, r1);

    if (r1) {
        int selectedIdx = ListView_GetNextItem(hList, -1, LVNI_SELECTED);
        EnableWindow(hNextBtn, selectedIdx != -1);
    }
    else if (r2) {
        EnableWindow(hNextBtn, TRUE);
    }
    else {
        EnableWindow(hNextBtn, FALSE);
    }
}

void LoadDriverAndRefresh(HWND hDlg) {
    int msgResult = MessageBoxW(
        hDlg,
        L"Please insert the installation media for the device and click OK to select the driver.",
        L"Add Drivers",
        MB_OKCANCEL
    );

    if (msgResult != IDOK) return;

    WCHAR szFile[MAX_PATH] = { 0 };
    OPENFILENAMEW ofn = { 0 };
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = hDlg;
    ofn.lpstrFilter = L"Setup Information (*.inf)\0*.inf\0All Files (*.*)\0*.*\0\0";
    ofn.lpstrFile = szFile;
    ofn.nMaxFile = MAX_PATH;
    ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY;
    ofn.lpstrTitle = L"Select Driver";

    if (GetOpenFileNameW(&ofn)) {
        wstring cmdLine = L"drvload.exe \"" + wstring(szFile) + L"\"";

        STARTUPINFOW si = { sizeof(si) };
        PROCESS_INFORMATION pi;
        si.cb = sizeof(si);
        si.dwFlags = STARTF_USESHOWWINDOW;
        si.wShowWindow = SW_HIDE;

        wchar_t cmdBuffer[MAX_PATH * 2];
        wcscpy_s(cmdBuffer, cmdLine.c_str());

        if (CreateProcessW(NULL, cmdBuffer, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
            WaitForSingleObject(pi.hProcess, INFINITE);
            CloseHandle(pi.hProcess);
            CloseHandle(pi.hThread);

            HWND hList = GetDlgItem(hDlg, IDC_LIST3);
            if (hList) {
                ListView_DeleteAllItems(hList);
                vector<OSInfo> detectedOSList = ScanInstalledOSList();

                for (size_t i = 0; i < detectedOSList.size(); ++i) {
                    LVITEM lvi = { 0 };
                    lvi.mask = LVIF_TEXT;
                    lvi.iItem = (int)i;
                    lvi.iSubItem = 0;
                    lvi.pszText = (LPTSTR)detectedOSList[i].name.c_str();
                    ListView_InsertItem(hList, &lvi);

                    ListView_SetItemText(hList, (int)i, 1, (LPTSTR)detectedOSList[i].sizeMB.c_str());
                    ListView_SetItemText(hList, (int)i, 2, (LPTSTR)detectedOSList[i].driveLetter.c_str());
                }

                if (ListView_GetItemCount(hList) > 0) {
                    EnableWindow(GetDlgItem(hDlg, IDC_RADIO1), TRUE);
                    CheckRadioButton(hDlg, IDC_RADIO1, IDC_RADIO2, IDC_RADIO1);
                }
                UpdateUIState(hDlg);
            }
        }
        else {
            MessageBoxW(hDlg, L"Failed to execute drvload.exe.", L"Error", MB_OK | MB_ICONERROR);
        }
    }
}


INT_PTR CALLBACK OSListDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        addCornerIcon(hDlg, IDI_ICON1);

        HWND hList = GetDlgItem(hDlg, IDC_LIST3);
        if (hList) {
            ListView_SetExtendedListViewStyle(hList, LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);

            LVCOLUMN lvc = { 0 };
            lvc.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;

            lvc.iSubItem = 0; lvc.pszText = (LPTSTR)TEXT("Operating System"); lvc.cx = 140;
            ListView_InsertColumn(hList, 0, &lvc);

            lvc.iSubItem = 1; lvc.pszText = (LPTSTR)TEXT("Partition Size"); lvc.cx = 90;
            ListView_InsertColumn(hList, 1, &lvc);

            lvc.iSubItem = 2; lvc.pszText = (LPTSTR)TEXT("Location"); lvc.cx = 110;
            ListView_InsertColumn(hList, 2, &lvc);

            vector<OSInfo> detectedOSList = ScanInstalledOSList();

            for (size_t i = 0; i < detectedOSList.size(); ++i) {
                LVITEM lvi = { 0 };
                lvi.mask = LVIF_TEXT;
                lvi.iItem = (int)i;
                lvi.iSubItem = 0;
                lvi.pszText = (LPTSTR)detectedOSList[i].name.c_str();
                ListView_InsertItem(hList, &lvi);

                ListView_SetItemText(hList, (int)i, 1, (LPTSTR)detectedOSList[i].sizeMB.c_str());
                ListView_SetItemText(hList, (int)i, 2, (LPTSTR)detectedOSList[i].driveLetter.c_str());
            }
        }

        if (ListView_GetItemCount(hList) == 0) {
            EnableWindow(GetDlgItem(hDlg, IDC_RADIO1), FALSE);
            CheckRadioButton(hDlg, IDC_RADIO1, IDC_RADIO2, IDC_RADIO2);
        }
        else {
            CheckRadioButton(hDlg, IDC_RADIO1, IDC_RADIO2, IDC_RADIO1);
        }


        UpdateUIState(hDlg);

        CenterWindowOnScreen(hDlg);

        return (INT_PTR)TRUE;
    }

    case WM_NOTIFY:
    {
        LPNMHDR pnmh = (LPNMHDR)lParam;
        if (pnmh->idFrom == IDC_LIST3 && pnmh->code == LVN_ITEMCHANGED)
        {
            LPNMLISTVIEW pNmlv = (LPNMLISTVIEW)lParam;
            if (pNmlv->uChanged & LVIF_STATE)
            {
                UpdateUIState(hDlg);
            }
        }
        break;
    }

    case WM_COMMAND:
    {
        int wmId = LOWORD(wParam);

        if (wmId == IDC_BUTTON1)
        {
            LoadDriverAndRefresh(hDlg);
            return (INT_PTR)TRUE;
        }

        if (wmId == IDC_RADIO1)
        {
            HWND hList = GetDlgItem(hDlg, IDC_LIST3);

            ListView_SetItemState(hList, -1, 0, LVIS_SELECTED);
            UpdateUIState(hDlg);
            return (INT_PTR)TRUE;
        }
        else if (wmId == IDC_RADIO2)
        {
            HWND hList = GetDlgItem(hDlg, IDC_LIST3);

            ListView_SetItemState(hList, -1, 0, LVIS_SELECTED);

            UpdateUIState(hDlg);
            return (INT_PTR)TRUE;
        }

        if (wmId == IDC_BUTTON2 || wmId == IDOK) // Selection
        {
            HWND hList = GetDlgItem(hDlg, IDC_LIST3);

            int selectedIdx = ListView_GetNextItem(hList, -1, LVNI_SELECTED);
            if (selectedIdx == -1) selectedIdx = 0;

            if (ListView_GetItemCount(hList) > 0) {
                wchar_t osName[256] = { 0 };
                wchar_t osLoc[256] = { 0 };

                ListView_GetItemText(hList, selectedIdx, 0, osName, 256);
                ListView_GetItemText(hList, selectedIdx, 2, osLoc, 256);

                wstring wFull = wstring(osName) + L" on " + wstring(osLoc);
                g_selectedOSInfo = string(wFull.begin(), wFull.end());
            }

            EndDialog(hDlg, IDOK);
            return (INT_PTR)TRUE;
        }
        else if (wmId == IDCANCEL)
        {
            EndDialog(hDlg, IDCANCEL);
            return (INT_PTR)TRUE;
        }
        break;
    }
    case WM_DESTROY:
    {
        GetWindowRect(hDlg, &g_lastDlgRect);
        break;
    }
    }

    return (INT_PTR)FALSE;
}

#define OS_INDICATIONS_GUID L"{8BE4DF61-93CA-11D2-AA0D-00E098032B8C}"
#define EFI_OS_INDICATIONS_BOOT_TO_FW_UI 0x0000000000000001ULL

static BOOL EnableSystemEnvironmentPrivilege(void) {
    HANDLE hToken;
    TOKEN_PRIVILEGES tp;

    if (!OpenProcessToken(GetCurrentProcess(),
        TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken))
        return FALSE;

    if (!LookupPrivilegeValueW(NULL, SE_SYSTEM_ENVIRONMENT_NAME, &tp.Privileges[0].Luid)) {
        CloseHandle(hToken);
        return FALSE;
    }

    tp.PrivilegeCount = 1;
    tp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;

    BOOL ok = AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(tp), NULL, NULL);
    DWORD err = GetLastError();
    CloseHandle(hToken);
    return ok && (err == ERROR_SUCCESS);
}

static BOOL SetBootToFirmwareUI(void) {
    if (!EnableSystemEnvironmentPrivilege()) {
        return FALSE;
    }

    DWORD64 value = 0;
    GetFirmwareEnvironmentVariableExW(
        L"OsIndications", OS_INDICATIONS_GUID,
        &value, sizeof(value), NULL);

    value |= EFI_OS_INDICATIONS_BOOT_TO_FW_UI;

    DWORD attributes = VARIABLE_ATTRIBUTE_NON_VOLATILE |
        VARIABLE_ATTRIBUTE_BOOTSERVICE_ACCESS |
        VARIABLE_ATTRIBUTE_RUNTIME_ACCESS;

    return SetFirmwareEnvironmentVariableExW(
        L"OsIndications", OS_INDICATIONS_GUID,
        &value, sizeof(value), attributes);
}

struct BCDSettings {
    int timeout = 30;              
    bool isLegacy = false;         
    bool alwaysShowMenu = false;   
    bool enableEditOptions = false;
};

BCDSettings ReadCurrentBCDSettings() {
    BCDSettings settings;

    wchar_t tempPath[MAX_PATH];
    wchar_t tempFile[MAX_PATH];
    GetTempPathW(MAX_PATH, tempPath);
    swprintf_s(tempFile, L"%sbcdsettings.txt", tempPath);

    wchar_t cmd[MAX_PATH * 2];
    swprintf_s(cmd, L"cmd.exe /c bcdedit /enum all > \"%s\"", tempFile);

    STARTUPINFOW si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    if (CreateProcessW(NULL, cmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        WaitForSingleObject(pi.hProcess, 3000);
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }

    wifstream file(tempFile);
    if (!file.is_open()) return settings;

    wstring line;
    while (getline(file, line)) {
        // Timeout
        if (line.find(L"timeout") != wstring::npos) {
            size_t pos = line.find_first_of(L"0123456789");
            if (pos != wstring::npos) {
                settings.timeout = _wtoi(line.substr(pos).c_str());
            }
        }
        // Legacy Boot
        if (line.find(L"bootmenupolicy") != wstring::npos) {
            if (line.find(L"Legacy") != wstring::npos || line.find(L"legacy") != wstring::npos) {
                settings.isLegacy = true;
            }
        }
        //  Always show boot menu
        if (line.find(L"displaybootmenu") != wstring::npos) {
            if (line.find(L"Yes") != wstring::npos || line.find(L"yes") != wstring::npos) {
                settings.alwaysShowMenu = true;
            }
        }
        //  Enable editing boot options
        if (line.find(L"optionsedit") != wstring::npos) {
            if (line.find(L"Yes") != wstring::npos || line.find(L"yes") != wstring::npos ||
                line.find(L"True") != wstring::npos || line.find(L"true") != wstring::npos) {
                settings.enableEditOptions = true;
            }
        }
    }

    file.close();
    DeleteFileW(tempFile);
    return settings;
}

INT_PTR CALLBACK BootDefaultsDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        CenterWindowOnScreen(hDlg);
        addCornerIcon(hDlg, IDI_ICON1);

        HWND hComboOS = GetDlgItem(hDlg, IDC_COMBO3);
        if (hComboOS) {
            SendMessage(hComboOS, CB_RESETCONTENT, 0, 0);
            g_osListForBootDefaults = ScanInstalledOSList();

            for (size_t i = 0; i < g_osListForBootDefaults.size(); ++i) {
                LRESULT idx = SendMessage(hComboOS, CB_ADDSTRING, 0, (LPARAM)g_osListForBootDefaults[i].name.c_str());
                SendMessage(hComboOS, CB_SETITEMDATA, idx, (LPARAM)i);
            }
            if (!g_osListForBootDefaults.empty()) {
                SendMessage(hComboOS, CB_SETCURSEL, 0, 0);
            }
        }

        BCDSettings currentBCD = ReadCurrentBCDSettings();

        HWND hComboTimer = GetDlgItem(hDlg, IDC_COMBO4);
        if (hComboTimer) {
            SendMessage(hComboTimer, CB_RESETCONTENT, 0, 0);

            LRESULT idx5 = SendMessage(hComboTimer, CB_ADDSTRING, 0, (LPARAM)L"5 Seconds");
            LRESULT idx30 = SendMessage(hComboTimer, CB_ADDSTRING, 0, (LPARAM)L"30 Seconds");
            LRESULT idx300 = SendMessage(hComboTimer, CB_ADDSTRING, 0, (LPARAM)L"5 Minutes");

            SendMessage(hComboTimer, CB_SETITEMDATA, idx5, 5);
            SendMessage(hComboTimer, CB_SETITEMDATA, idx30, 30);
            SendMessage(hComboTimer, CB_SETITEMDATA, idx300, 300);

            if (currentBCD.timeout <= 5) {
                SendMessage(hComboTimer, CB_SETCURSEL, idx5, 0);
            }
            else if (currentBCD.timeout >= 300) {
                SendMessage(hComboTimer, CB_SETCURSEL, idx300, 0);
            }
            else {
                SendMessage(hComboTimer, CB_SETCURSEL, idx30, 0);
            }
        }

        CheckDlgButton(hDlg, IDC_CHECK1, currentBCD.isLegacy ? BST_CHECKED : BST_UNCHECKED);
        CheckDlgButton(hDlg, IDC_CHECK2, currentBCD.alwaysShowMenu ? BST_CHECKED : BST_UNCHECKED);
        CheckDlgButton(hDlg, IDC_CHECK3, currentBCD.enableEditOptions ? BST_CHECKED : BST_UNCHECKED);

        return (INT_PTR)TRUE;
    }

    case WM_COMMAND:
    {
        int wmId = LOWORD(wParam);

        if (wmId == IDOK)
        {
            HWND hComboOS = GetDlgItem(hDlg, IDC_COMBO3);
            LRESULT selIdx = SendMessage(hComboOS, CB_GETCURSEL, 0, 0);
            if (selIdx != CB_ERR) {
                LRESULT dataIdx = SendMessage(hComboOS, CB_GETITEMDATA, selIdx, 0);
                if (dataIdx >= 0 && dataIdx < (LRESULT)g_osListForBootDefaults.size()) {
                    wstring targetGUID = g_osListForBootDefaults[dataIdx].guid;
                    wchar_t cmdBuf[MAX_PATH];
                    swprintf_s(cmdBuf, L"bcdedit /default %s", targetGUID.c_str());
                    char szCmd[MAX_PATH];
                    WideCharToMultiByte(CP_ACP, 0, cmdBuf, -1, szCmd, sizeof(szCmd), NULL, NULL);
                    RunCommand(szCmd);
                }
            }

            HWND hComboTimer = GetDlgItem(hDlg, IDC_COMBO4);
            LRESULT timerSel = SendMessage(hComboTimer, CB_GETCURSEL, 0, 0);
            if (timerSel != CB_ERR) {
                LRESULT seconds = SendMessage(hComboTimer, CB_GETITEMDATA, timerSel, 0);
                wchar_t cmdBuf[MAX_PATH];
                swprintf_s(cmdBuf, L"bcdedit /timeout %d", (int)seconds);
                char szCmd[MAX_PATH];
                WideCharToMultiByte(CP_ACP, 0, cmdBuf, -1, szCmd, sizeof(szCmd), NULL, NULL);
                RunCommand(szCmd);
            }

            if (IsDlgButtonChecked(hDlg, IDC_CHECK1) == BST_CHECKED) {
                RunCommand("bcdedit /set {default} bootmenupolicy legacy");
            }
            else {
                RunCommand("bcdedit /set {default} bootmenupolicy standard");
            }

            if (IsDlgButtonChecked(hDlg, IDC_CHECK2) == BST_CHECKED) {
                RunCommand("bcdedit /set {bootmgr} displaybootmenu yes");
            }
            else {
                RunCommand("bcdedit /set {bootmgr} displaybootmenu no");
            }

            if (IsDlgButtonChecked(hDlg, IDC_CHECK3) == BST_CHECKED) {
                RunCommand("bcdedit /set {globalsettings} optionsedit true");
            }
            else {
                RunCommand("bcdedit /set {globalsettings} optionsedit false");
            }

            MessageBoxW(hDlg, L"Successfully updated boot defaults!", L"Change Boot Defaults", MB_OK | MB_ICONINFORMATION);
            EndDialog(hDlg, IDOK);
            return (INT_PTR)TRUE;
        }
        else if (wmId == IDCANCEL)
        {
            EndDialog(hDlg, IDCANCEL);
            return (INT_PTR)TRUE;
        }
        break;
    }
    }
    return (INT_PTR)FALSE;
}

void BootToEFIDevice(const wstring& keyword) {
    wchar_t tempPath[MAX_PATH];
    wchar_t tempFile[MAX_PATH];
    GetTempPathW(MAX_PATH, tempPath);
    swprintf_s(tempFile, L"%sfwboot.txt", tempPath);

    wchar_t cmd[MAX_PATH * 2];
    swprintf_s(cmd, L"cmd.exe /c bcdedit /enum firmware > \"%s\"", tempFile);

    STARTUPINFOW si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    si.cb = sizeof(si);
    if (CreateProcessW(NULL, cmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        WaitForSingleObject(pi.hProcess, 3000);
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }

    wifstream file(tempFile);
    wstring targetGuid = L"";

    if (file.is_open()) {
        wstring line;
        wstring currentGuid = L"";

        while (getline(file, line)) {
            size_t idPos = line.find(L"identifier");
            if (idPos != wstring::npos) {
                wstring val = line.substr(idPos + 10);
                size_t first = val.find_first_not_of(L" \t\r\n");
                size_t last = val.find_last_not_of(L" \t\r\n");
                if (first != wstring::npos && last != wstring::npos) {
                    currentGuid = val.substr(first, (last - first + 1));
                }
            }

            size_t descPos = line.find(L"description");
            if (descPos != wstring::npos) {
                wstring desc = line.substr(descPos + 11);
                auto it = search(
                    desc.begin(), desc.end(),
                    keyword.begin(), keyword.end(),
                    [](wchar_t c1, wchar_t c2) { return towlower(c1) == towlower(c2); }
                );
                if (it != desc.end() && !currentGuid.empty()) {
                    targetGuid = currentGuid;
                    break;
                }
            }
        }
        file.close();
        DeleteFileW(tempFile);
    }

    if (!targetGuid.empty()) {
        wchar_t setCmd[MAX_PATH];
        swprintf_s(setCmd, L"bcdedit /set {fwbootmgr} bootsequence %s", targetGuid.c_str());
        char szCmd[MAX_PATH];
        WideCharToMultiByte(CP_ACP, 0, setCmd, -1, szCmd, sizeof(szCmd), NULL, NULL);
        RunCommand(szCmd);
    }

    RunCommand("wpeutil reboot");
}

static HFONT g_hUseDeviceFont = NULL;

INT_PTR CALLBACK UseDeviceDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        CenterWindowOnScreen(hDlg);
        addCornerIcon(hDlg, IDI_ICON1);

        HDC hdc = GetDC(hDlg);
        int fontHeight = -MulDiv(12, GetDeviceCaps(hdc, LOGPIXELSY), 72);
        ReleaseDC(hDlg, hdc);

        g_hUseDeviceFont = CreateFontW(
            fontHeight,             
            0, 0, 0,
            FW_NORMAL,              
            FALSE, FALSE, FALSE,
            DEFAULT_CHARSET,
            OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
            DEFAULT_QUALITY,
            DEFAULT_PITCH | FF_DONTCARE,
            L"Microsoft JhengHei"   
        );

        SendDlgItemMessage(hDlg, IDC_TITLE_USE_DEVICE, WM_SETFONT, (WPARAM)g_hUseDeviceFont, TRUE);

        return (INT_PTR)TRUE;
    }

    case WM_CTLCOLORSTATIC:
    {
        HDC hdcStatic = (HDC)wParam;
        HWND hwndStatic = (HWND)lParam;

        if (GetDlgCtrlID(hwndStatic) == IDC_TITLE_USE_DEVICE)
        {
            SetTextColor(hdcStatic, RGB(0, 57, 187)); 
            SetBkMode(hdcStatic, TRANSPARENT);
            return (INT_PTR)GetStockObject(NULL_BRUSH);
        }
        break;
    }

    case WM_COMMAND:
    {
        int wmId = LOWORD(wParam);
        switch (wmId)
        {
        case IDC_COMMAND1: // EFI USB Devices
            BootToEFIDevice(L"USB");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case IDC_COMMAND2: // EFI CD/DVD Drives
            BootToEFIDevice(L"CD");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case IDC_COMMAND3: // EFI Network
            BootToEFIDevice(L"Network");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case IDCANCEL:
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;
        }
        break;
    }

    case WM_DESTROY:
    {
        if (g_hUseDeviceFont) {
            DeleteObject(g_hUseDeviceFont);
            g_hUseDeviceFont = NULL;
        }
        break;
    }
    }
    return (INT_PTR)FALSE;
}


INT_PTR CALLBACK MyDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        HBITMAP hBmp = (HBITMAP)LoadImage(
            GetModuleHandle(NULL),
            MAKEINTRESOURCE(IDB_BITMAP1),
            IMAGE_BITMAP,
            0, 0, 
            LR_CREATEDIBSECTION
        );

        if (hBmp) {
            BITMAP bmp = { 0 };
            GetObject(hBmp, sizeof(BITMAP), &bmp);

            HWND hTitleWnd = GetDlgItem(hDlg, IDC_TITLE_TEXT);
            SetWindowPos(hTitleWnd, NULL, 0, 0, bmp.bmWidth, bmp.bmHeight, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);

            SendDlgItemMessage(hDlg, IDC_TITLE_TEXT, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBmp);
        }

        string osDetectedInfo = g_selectedOSInfo;
        string finalDisplayString = "Operating system: " + osDetectedInfo;
        SetDlgItemTextA(hDlg, IDC_OS_INFO_TEXT, finalDisplayString.c_str());

        HWND hTitleText = GetDlgItem(hDlg, IDC_TITLE_TEXT);
        if (hTitleText) {
            SendMessage(hTitleText, WM_SETFONT, (WPARAM)g_hTitleFont, TRUE);
        }

        addCornerIcon(hDlg, IDI_ICON1);

        CenterWindowOnScreen(hDlg);

        return (INT_PTR)TRUE;
    }

    case WM_NOTIFY:
    {

        LPNMHDR pnmh = (LPNMHDR)lParam;

        if (pnmh->idFrom == IDC_SPLIT1 && pnmh->code == BCN_DROPDOWN)
        {
            //Other boot options
            HMENU hPopupMenu = CreatePopupMenu();
            AppendMenu(hPopupMenu, MF_STRING, 1001, L"Startup Settings");
            AppendMenu(hPopupMenu, MF_STRING, 1002, L"UEFI Firmware Settings");
            AppendMenu(hPopupMenu, MF_SEPARATOR, 1003, L"");
            AppendMenu(hPopupMenu, MF_STRING, 1004, L"Use a Device");
            AppendMenu(hPopupMenu, MF_SEPARATOR, 1005, L"");
            AppendMenu(hPopupMenu, MF_STRING, 1006, L"Change Boot Defaults");

            NMBCDROPDOWN* pDropDown = (NMBCDROPDOWN*)lParam;
            POINT pt = { pDropDown->rcButton.left, pDropDown->rcButton.bottom };
            ClientToScreen(pnmh->hwndFrom, &pt);

            TrackPopupMenu(hPopupMenu, TPM_LEFTALIGN | TPM_TOPALIGN, pt.x, pt.y, 0, hDlg, NULL);

            DestroyMenu(hPopupMenu);
            return (INT_PTR)TRUE;
        }

        if (pnmh->code == NM_CLICK || pnmh->code == NM_RETURN)
        {
            switch (pnmh->idFrom)
            {
            case IDC_SYSLINK1: // Change language
            case IDC_SYSLINK7:
                DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG3), hDlg, AccessDeniedDlgProc);
                return (INT_PTR)TRUE;

            case IDC_SYSLINK2: // Startup Repair
                RunCommand("X:\\sources\\recovery\\StartRep.exe");
                return (INT_PTR)TRUE;

            case IDC_SYSLINK3: // System Restore
            {
                char driveLetter = 'C';

                size_t pos = g_selectedOSInfo.find('(');
                if (pos != string::npos && pos + 1 < g_selectedOSInfo.length()) {
                    driveLetter = g_selectedOSInfo[pos + 1];
                }

                string cmd = "X:\\windows\\system32\\rstrui.exe /offline:" + string(1, driveLetter) + ":\\Windows";
                RunCommand(cmd.c_str());
                return (INT_PTR)TRUE;
            }

            case IDC_SYSLINK4: // System Image Recovery
                RunCommand("X:\\windows\\system32\\bmrui.exe");
                return (INT_PTR)TRUE;

            case IDC_SYSLINK5: // Windows Memory Diagnostic
                RunCommand("X:\\windows\\system32\\mdsched.exe");
                return (INT_PTR)TRUE;

            case IDC_SYSLINK6: // Command Prompt
                WinExec("cmd.exe", SW_SHOW);
                return (INT_PTR)TRUE;

            case IDC_SYSLINK8: // RecEnv.exe
            {
                HANDLE hFile = CreateFileA("C:\\use_recenv.tag", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                if (hFile != INVALID_HANDLE_VALUE) {
                    CloseHandle(hFile); 
                }

                RunCommand("reagentc /boottore");
                RunCommand("wpeutil reboot");

                EndDialog(hDlg, IDOK);
                return (INT_PTR)TRUE;
            }
            }
        }
        break;
    }

    case WM_COMMAND:
    {
        int wmId = LOWORD(wParam);

        switch (wmId)
        {
        case IDC_BUTTON1: // Shut Down
            RunCommand("wpeutil shutdown");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case IDC_SPLIT1: // Restart
            RunCommand("wpeutil reboot");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case 1001:

            RunCommand("bcdedit /set {default} onetimeadvancedoptions on");

            RunCommand("wpeutil reboot");
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;

        case 1002:

            if (SetBootToFirmwareUI()) {
                RunCommand("wpeutil reboot");
                EndDialog(hDlg, wmId);
            }
            else {
                MessageBoxA(hDlg,
                    "Cannot automatically enter BIOS settings."
                    "Please restart normally instead.",
                    "Error", MB_OK | MB_ICONWARNING);
            }

            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;


        case 1004:
        {
            DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG6), hDlg, UseDeviceDlgProc);
            return (INT_PTR)TRUE;
        }

        case 1006:
        {
            DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG5), hDlg, BootDefaultsDlgProc);
            return (INT_PTR)TRUE;
        }

        case IDCANCEL:
            EndDialog(hDlg, wmId);
            return (INT_PTR)TRUE;
        }
        break;
    }

    case WM_DESTROY:
        if (g_hTitleFont) {
            DeleteObject(g_hTitleFont);
        }
        break;
    }
    return (INT_PTR)FALSE;
}

void RunAndBlock(const char* cmd) {
    STARTUPINFOA si;
    PROCESS_INFORMATION pi;
    ZeroMemory(&si, sizeof(si));
    si.cb = sizeof(si);
    ZeroMemory(&pi, sizeof(pi));

    char cmdBuffer[MAX_PATH];
    strcpy_s(cmdBuffer, cmd);

    if (CreateProcessA(NULL, cmdBuffer, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        WaitForSingleObject(pi.hProcess, INFINITE);
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }
    //PREVENT RETURN 0 AFTER RECENV.EXE STARTED!!! DO NOT DELETE!!!
}

// Sequence of opening different windows
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    _In_opt_ HINSTANCE hPrevInstance,
    _In_ LPWSTR    lpCmdLine,
    _In_ int       nCmdShow)
 {
    UNREFERENCED_PARAMETER(hPrevInstance);
    UNREFERENCED_PARAMETER(lpCmdLine);
    UNREFERENCED_PARAMETER(nCmdShow);

    SetThemeAppProperties(0);

    if (GetFileAttributesA("C:\\use_recenv.tag") != INVALID_FILE_ATTRIBUTES) {
        DeleteFileA("C:\\use_recenv.tag");
        RunAndBlock("X:\\sources\\recovery\\recenv.exe");
        return 0;
    }

    INITCOMMONCONTROLSEX icex;
    icex.dwSize = sizeof(INITCOMMONCONTROLSEX);
    icex.dwICC = ICC_LINK_CLASS | ICC_LISTVIEW_CLASSES;
    InitCommonControlsEx(&icex);

    INT_PTR langRet = DialogBox(hInstance, MAKEINTRESOURCE(IDD_DIALOG2), NULL, LangDlgProc);

    if (langRet == IDOK) {
        INT_PTR osListRet = DialogBox(hInstance, MAKEINTRESOURCE(IDD_DIALOG4), NULL, OSListDlgProc);

        if (osListRet == IDOK) {
            DialogBox(hInstance, MAKEINTRESOURCE(IDD_DIALOG1), NULL, MyDialogProc);
        }
    }

    return 0;
}